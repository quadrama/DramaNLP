...
public class TestGraphIO {
	...
	public void setUp() throws UIMAException {
		...
		for (int i = 0; i < figures.length; i++) {
			figures[i] =
					AnnotationFactory.createAnnotation(jcas, i * 2, i * 2 + 1,
							Figure.class);
			figures[i].setId(i);
		}
	...
	@Test
	public void testGraphIO() throws IOException, CASException,
		...
		for (int i = 0; i < figures.length - 1; i++) {
			DefaultWeightedEdge edge =
					graph.addEdge(figures[i], figures[i + 1]);
		...
		Graph<Figure, DefaultWeightedEdge> g2 =
				GraphImporter.getGraph(jcas, "Testview");

		for (int i = 0; i < figures.length; i++) {
			for (int j = 0; j < figures.length; j++) {
				assertEquals(graph.containsEdge(figures[i], figures[j]),
						g2.containsEdge(figures[i], figures[j]));
				if (graph.containsEdge(figures[i], figures[j])) {
					assertEquals(graph.getEdgeWeight(graph.getEdge(figures[i],
							figures[j])), g2.getEdgeWeight(g2.getEdge(
							figures[i], figures[j])), 1e-3);
				}
			}
		}
